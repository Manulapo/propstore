...
export const config = {
  ...
  callbacks: {
    async session({ session, user, trigger, token }: any) { // This is a callback that is used to create the session. It is called when the user is authenticated. It is also called when the user is updated.
      if (!token) {
    ...
    async jwt({ token, user }: any) { // This is a callback that is used to create the token.
      if (user) {
        token.id = user.id;
    ...
    authorized({ request }: any) {// This is a callback that is used to check if the user is authorized to access the page.
